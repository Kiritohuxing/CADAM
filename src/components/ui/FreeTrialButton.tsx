...
const FreeTrialButton: React.FC<FreeTrialButtonProps> = ({
  ...
  const generateParticles = () => {
    return Array.from({ length: 15 }).map(() => ({
      ...
      originX: Math.random() * 1000,
      originY: Math.random() * 1000,
    }));
  };

  const particles = React.useMemo(() => generateParticles(), []);

  // Only show animations if hovered AND not in loading/disabled state
  const isActive = isHovered && !isPending && !disabled;

  return (
    <div className={cn('sparkle-button', className)}>
      <button
        onClick={onClick}
        ...
        style={
          {
            '--active': isActive ? '1' : '0',
            // Simple background during loading state
            background: isPending ? 'hsl(0, 0%, 13%)' : undefined,
      ...
      >
        ...
        {/* Sparkle SVG - only show when not loading */}
        {!isPending && (
          <svg
            viewBox="0 0 24 24"
            fill="none"
        ...
        <span
          className={cn(
            'text',
            isPending && 'loading-text', // Add a class for loading state
          )}
        >
          {isPending ? (
            <>
      ...
      {/* Only show these effects when not in loading state */}
      {!isPending && (
        <>
          {/* Body drop */}
