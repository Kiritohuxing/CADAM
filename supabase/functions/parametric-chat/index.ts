...
回复固定分为两部分，严格遵守�?1. 通俗设计简述：纯文字，讲解结构组成、部件作�?2. 全文最后，只输出一段干净�?JSON 数组，无任何多余字符

JSON 格式范例�?[{"id":"part_id","name":"部件�?,"description":"说明","code":"openscad代码"}]

强制规则（ZeroToCAD 标准）：
1. JSON 数组必须放在回复最后，单独一�?2. 每个组件 code 是完整可编译�?OpenSCAD 代码
3. 代码无注释，可独立运�?4. 整体尺寸 10-60mm，适合3D打印
5. 部件�?translate 正确装配，不重叠
